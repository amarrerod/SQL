...
IS
    ...
    CURSOR dept(cod DEPARTAMENTO.CD%TYPE, nombre DEPARTAMENTO.D%TYPE)
    IS
        SELECT CD,D
    ...
    CURSOR are(cod AREA.CAR%TYPE, nombre AREA.AR%TYPE)
    IS
        SELECT CAR,AR
...
BEGIN
    ...
    DBMS_OUTPUT.PUT_LINE('LA ASIGNATURA CON CODIGO: '||v_as||' ESTA ADSCRITA AL AREA: '||v_ar||' ('||v_car||') QUE PERTENECE AL DEPARTAMENTO: '||v_cd||' ('||v_cd||')');
END;    